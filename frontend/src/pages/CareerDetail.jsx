...
import AttemptsCalculator from '../components/AttemptsCalculator'
  ...
  return (
    <div className="container career-detail fade-in" style={{ padding: '32px 24px 80px' }}>
      ...
      {career.career_roadmap && (
        <div className="career-detail__roadmap card">
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-accent)', textTransform: 'uppercase', marginBottom: 6 }}>
            Career Roadmap
          </div>
          <p style={{ margin: 0, fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 600, color: 'var(--text-primary)' }}>
            {career.career_roadmap}
          </p>
        </div>
      )}
      ...
      <div className="career-detail__stats-row">
        <StatCard icon="graduation-cap" label="Eligibility to Apply" value={career.min_education} />
        <StatCard icon="award" label="Required Degree" value={career.required_degree || 'See qualifications below'} small />
        <StatCard icon="clock" label="Course Duration" value={career.course_duration || 'Varies'} />
        <StatCard icon="compass" label="Category" value={career.category} />
      </div>

      {career.min_education === '12th' && career.required_degree && (
        <p className="career-detail__eligibility-note">
          <Icon name="bell" size={14} /> "Eligibility to Apply" is the school-stage entry point, not the finished
          qualification - see <strong>Required Degree</strong> above and the roadmap for the full path.
        </p>
      )}
      ...
      <Section title="Required Qualifications" icon="graduation-cap">
        ...
        {career.licensing_requirements && (
          <div className="career-detail__licensing">
            <strong>Licensing / Registration:</strong> {career.licensing_requirements}
          </div>
        )}
      </Section>

      <Section title="Entrance Exams & Admission Pathways" icon="award">
        {career.entrance_exams.length > 1 && (
          <p style={{ fontSize: 13, marginTop: -6, marginBottom: 16 }}>
            Multiple valid pathways exist - you don't need to clear all of these, just one.
          </p>
        )}

        <AttemptsCalculator exams={career.entrance_exams} />
      ...
      <Section title="Salary Range" icon="chart">
        <div className="career-detail__salary-grid">
          <SalaryTier label="Entry" value={career.salary_range.entry} />
          <SalaryTier label="Average" value={career.salary_range.average} />
          <SalaryTier label="Mid-Career" value={career.salary_range.mid} />
          <SalaryTier label="Senior" value={career.salary_range.senior} />
          <SalaryTier label="Highest Potential" value={career.salary_range.highest_potential} highlight />
        </div>
        {career.salary_range.note && (
          <p className="career-detail__salary-note">
            <Icon name="bell" size={13} /> {career.salary_range.note}
          </p>
        )}
      </Section>
      ...
      <div className="career-detail__two-col">
        <Section title="Government Opportunities" icon="landmark">
          <p>{career.government_opportunities}</p>
        </Section>
        <Section title="Private Opportunities" icon="briefcase">
          <p>{career.private_opportunities}</p>
        </Section>
      </div>

      <div className="career-detail__two-col">
        <Section title="Future Scope" icon="chart">
          <p>{career.future_scope}</p>
        </Section>
        <Section title="AI Impact" icon="bot">
          <p>{career.ai_impact}</p>
        </Section>
      </div>

      {career.required_skills?.length > 0 && (
        <Section title="Required Skills" icon="target">
          <div className="career-detail__chip-row">
            {career.required_skills.map((s, i) => <span key={i} className="career-detail__chip">{s}</span>)}
          </div>
        </Section>
      )}
      ...
      {career.certifications?.length > 0 && (
        <Section title="Certifications" icon="award">
          <div className="career-detail__chip-row">
            {career.certifications.map((c, i) => <span key={i} className="career-detail__chip career-detail__chip--accent">{c}</span>)}
          </div>
        </Section>
      )}

      {career.top_recruiters?.length > 0 && (
        <Section title="Top Recruiters" icon="briefcase">
          <div className="career-detail__chip-row">
            {career.top_recruiters.map((r, i) => <span key={i} className="career-detail__chip">{r}</span>)}
          </div>
        </Section>
      )}

      {career.higher_education_options?.length > 0 && (
        <Section title="Higher Education Options" icon="graduation-cap">
          <ul>{career.higher_education_options.map((h, i) => <li key={i}>{h}</li>)}</ul>
        </Section>
      )}
...
function StatCard({ icon, label, value, small }) {
  return (
    <div className="card career-detail__stat-card">
      ...
      <div>
        <div style={{ fontWeight: 700, fontSize: small ? 12.5 : 14, lineHeight: 1.3 }}>{value}</div>
...
function SalaryTier({ label, value, highlight }) {
  return (
    <div className={`career-detail__salary-tier ${highlight ? 'is-highlight' : ''}`}>
      <div className="career-detail__salary-tier-label">{label}</div>
      <div className="career-detail__salary-tier-value">{value}</div>
    </div>
  )
}
