...
ALIASES = {
    "software engineer": ["developer", "programmer", "swe", "coder", "software developer", "web developer", "app developer", "jee"],
    "doctor": ["mbbs", "physician", "medical doctor", "surgeon", "neet", "neet ug", "neet-ug"],
    "chartered accountant": ["ca", "accountant", "auditor", "icai", "ca foundation"],
    "civil services ias": ["ias", "ips", "civil services", "collector", "bureaucrat", "upsc", "upsc cse"],
    "data scientist": ["data science", "machine learning engineer", "ml engineer", "ai engineer", "data analyst"],
    "lawyer": ["advocate", "attorney", "legal", "law", "clat", "ailet", "llb"],
    "mechanical engineer": ["mechanical engineering"],
    "architect": ["architecture", "nata", "b arch"],
    "commercial pilot": ["pilot", "aviator", "airline pilot", "cpl"],
    "fashion designer": ["fashion design", "fashion designing", "nift"],
    "psychologist": ["psychology", "therapist", "counselor", "counsellor"],
    "teacher professor": ["teacher", "professor", "educator", "lecturer", "ctet", "tet", "ugc net", "net exam"],
    "journalist": ["journalism", "reporter", "news anchor", "media", "iimc"],
    "product manager": ["product management", "pm", "cat exam"],
    "army officer": [
    ...
    "pharmacist": ["pharmacy", "pharmaceutical", "gpat", "b pharm", "d pharm"],
    "state civil services": ["mpsc", "state psc", "deputy collector", "tehsildar", "state pcs", "uppsc", "bpsc"],
    "ssc cgl officer": [
        "ssc", "ssc cgl", "staff selection commission", "government job", "sarkari naukri",
        "govt job", "income tax inspector", "cgl",
    ],
    "ssc chsl officer": [
        "ssc chsl", "chsl", "lower division clerk", "ldc", "data entry operator",
    ...
    "dentist": ["bds", "dental surgeon", "dental doctor", "neet dental"],
    "nurse": ["nursing", "staff nurse", "bsc nursing", "gnm", "nursing neet"],
    ...
    "company secretary": ["cs", "company secretary", "corporate secretary", "icsi"],
    "bank po": ["bank job", "banking", "ibps", "sbi po", "probationary officer", "bank clerk", "ibps po"],
    "judge": ["judiciary", "judicial services", "magistrate", "civil judge"],
    "graphic uiux designer": ["graphic designer", "ui designer", "ux designer", "ui ux", "product designer", "visual designer", "nid"],
...
def _fuzzy_contains(alias: str, text: str) -> bool:
    """
    True if `alias` appears within `text` (or vice versa). Short strings
    (<=4 chars, e.g. "ca", "cs", "cat") use strict word-boundary matching so
    they can't accidentally match as a substring inside an unrelated word
    (e.g. "ca" inside "cat", "afcat" inside "ca"-adjacent text). Longer
    strings keep flexible substring matching, since accidental collisions
    are far less likely once there's more text to match against.
    """
    def _side_matches(needle: str, haystack: str) -> bool:
        if len(needle) <= 4:
            return re.search(rf"\b{re.escape(needle)}\b", haystack) is not None
        return needle in haystack

    return _side_matches(alias, text) or _side_matches(text, alias)
...
def find_best_career_match(dream_career_input: str, careers: list[dict]) -> dict | None:
    ...
    # 2b. Fuzzy/substring alias match - only reached if no exact alias matched
    # above. Uses word-boundary-aware matching (see _fuzzy_contains) so short
    # aliases like "ca" or "cs" can't falsely match inside words like "cat".
    for career in careers:
        key = career["slug"].replace("-", " ")
        aliases = ALIASES.get(key, [])
        if any(_fuzzy_contains(a, normalized_input) for a in aliases):
            return career
