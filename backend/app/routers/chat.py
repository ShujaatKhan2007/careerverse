...
from app.utils.chat_engine import generate_rule_based_response, generate_llm_response, test_gemini_connection, test_anthropic_connection
...
async def get_provider_status(current_user: dict = Depends(get_current_user)):
    """
    Diagnostic check for whether the configured AI provider (Gemini or
    Anthropic) is actually working - makes a real minimal test call to each
    configured provider and reports the result, including the specific
    error if one failed. Use this from /docs to debug "why isn't my API
    key working" without digging through server logs.
    """
    ...
    elif anthropic["working"]:
        active_engine = "anthropic"
    else:
        active_engine = "rule-based (free fallback - no working LLM provider configured)"

    return {"gemini": gemini, "anthropic": anthropic, "active_engine": active_engine}
